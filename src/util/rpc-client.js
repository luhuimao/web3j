...
RpcClient.prototype.divisionCommand = function divisionCommand(callback) {
    ...
    */
    ...
    var division = this.config.division;
    ...
    var divisionType = typeof division;
    if (divisionType !== 'number' && divisionType !== 'string') {
        //boolean, object/array, or function was passed for division
        self.logger.warn({
            passedDivision: division,
            divisionType: divisionType
        }, 'RpcClient.divisionCommand(%s:%s) - Divison passed was not a string compatible type', self.config.host, self.config.port);
    }
        ...
        if (result === '0\r\n') {
            ...
            self.sender.send(userInfoRpcString, function userInfoCallback(error, result) {
                ...
                if (String(divisionInfo[2]) === division) {
                    ...
                    }, 'RpcClient.divisionCommand(%s:%s) - Set division did not find the users division', self.config.host, self.config.port);
